...
class BindingEngine:
    ...
    def bind_agent_to_project(
        db: Session, project_id: str, agent_id: str, is_lead: bool = False
    ) -> ProjectAgent:
        """에이전트를 프로젝트에 바인딩
        ...
        binding = ProjectAgent(
            project_id=project_id, agent_id=agent_id, is_lead=is_lead
        )
        db.add(binding)
        db.commit()
        db.refresh(binding)
    ...
    @staticmethod
    def unbind_agent_from_project(db: Session, project_id: str, agent_id: str) -> bool:
        """프로젝트에서 에이전트 바인딩 해제
        ...
        if not binding:
            logger.warning(
                f"No binding found for agent {agent_id} in project {project_id}"
            )
            return False
    ...
    def get_project_agents(db: Session, project_id: str) -> List[ProjectAgent]:
        ...
        """
        bindings = (
            db.query(ProjectAgent).filter(ProjectAgent.project_id == project_id).all()
        )
    ...
    def get_agent_projects(db: Session, agent_id: str) -> List[ProjectAgent]:
        ...
        """
        bindings = (
            db.query(ProjectAgent).filter(ProjectAgent.agent_id == agent_id).all()
        )
    ...
    def get_project_lead_agents(db: Session, project_id: str) -> List[ProjectAgent]:
        ...
        bindings = (
            db.query(ProjectAgent)
            .filter(ProjectAgent.project_id == project_id, ProjectAgent.is_lead == True)
            .all()
        )
    ...
    @staticmethod
    def is_agent_bound_to_project(db: Session, project_id: str, agent_id: str) -> bool:
    ...
    @staticmethod
    def get_binding_with_agent_info(
        db: Session, project_id: str
    ) -> List[Tuple[ProjectAgent, Agent]]:
